    ...
    try:
        ...
        for item in output_data:
            ...
            row = {
                ...
                'Description': None, 'Industry': None, 'Sales Line': '',
            ...
            if original_row is not None:
                row.update({
                    ...
                    'Number': original_row.get('Number'), # Corrected column name
            ...
            if item:
                row.update({
                    ...
                    'Sales Line': item.phone_sales_line.replace('{programmatic placeholder}', f"{item.avg_leads_per_day:.0f}") if item.phone_sales_line and item.avg_leads_per_day is not None else item.phone_sales_line,
                    'Key Resonating Themes': "; ".join(item.match_rationale_features) if item.match_rationale_features else "",
                    'Matched Partner Name': item.matched_partner_name or '',
